...
| The image you right-click on (including the first frame of GIF/APNG/animated WebP and the current frame of `<video>`, captured to JPEG) | Yes | Input to the visual model | In-memory only, not written to disk | Sent only to the provider you selected |
