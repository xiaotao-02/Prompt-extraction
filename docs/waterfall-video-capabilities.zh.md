...
- **用户直接右键点在 `<video>` 上**时：`showFallback` 为 **false**（不点亮兜底菜单，避免与原生 `video` 菜单重复），但仍把抓拍 / poster / src 写入 **tab 级 `pendingTabExtract`**；后台在 **`添加到参考` 菜单 `onClicked`** 时 **10s 内优先**于 `info.srcUrl` 使用 **`data:image/jpeg` 抓拍**（或当 `srcUrl` 判为视频流时优先使用 prep 中的静态 URL），避免 `video/*` 无法在 Service Worker 解码的问题。
...
1. **MV3 Service Worker 竞态（兜底菜单）**  
   主菜单只在 Chrome 判定的 **图片 / 视频** 上下文显示（**一级「添加到参考」**，避免 Chrome 将本扩展两项折叠成「Prompt Extracto」子菜单）；遮罩下的图等依赖 **兜底菜单**，由后台在收到 **`CTX_MENU_PREP`**（`sendMessage` **+** keepalive **`port.postMessage` 双通道**）后 **`contextMenus.update`**。保活 Port 见 [`src/content/bgPort.ts`](../src/content/bgPort.ts)、[`src/lib/keepalivePort.ts`](../src/lib/keepalivePort.ts)。
...
**验收建议**：浏览器冷启动后立刻打开普通 HTTPS 图站，在**遮罩覆盖的图片**上第一次右键，应能稳定看到扩展兜底「添加到参考」；直接右键原生 `<img>` / `<video>` 仍应只看到 **一级**「添加到参考」（不与兜底重复同时出现）。
...
| 遮罩 `<img>`（Behance 类） | 透明层上右键 | 出现兜底「添加到参考」；图为栈内小面积 `currentSrc` |
| 原生 `<video>`（同站可抓帧） | 视频画面上右键 → 添加到参考 | 走 **JPEG**，后台不误走 `video/*` 下载报错 |
| `<video>` + 跨域污染 | 无 poster、抓帧失败 | 可能仍失败或落到视频 URL；提示与 README 一致 |
| `<video>` + `pointer-events:none` | 点在视频区域（命中矩形兜底） | `showFallback:true`，兜底「添加到参考」可用；图为帧或 poster |
