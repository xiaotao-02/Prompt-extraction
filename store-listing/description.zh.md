做 AI 绘画时，最费时间往往不是「画」，而是把一张参考图变成**可用、可复制、格式对口**的提示词。**Prompt Extracto** 让你在浏览网页时就能开工：看中参考图的造型、光影与构图，**右键即可**交给视觉模型把画面「读」成提示词，少在工具之间搬运文件、少一遍遍手敲标签。

你只需**自备模型服务的 API Key**、在扩展里任选一家供应商；扩展**不向开发者后端上传内容与密钥**，数据流细节见下文「隐私与数据传输」。装好以后，几秒内在页面右下角浮动面板拿到可复制结果；并可在面板内**对话改写、版本回溯**（具体输出样式与快捷键等见「主要特性」）。

输入涵盖 PNG / JPEG、GIF / APNG / 动画 WebP，以及 Twitter、Reddit、Discord 等平台常见的「假 GIF」`<video>` 元素等；画布、内联 SVG、CSS 背景图等同样在支持列表中。
...
Q：安装时出现「增强型安全浏览功能不信任此扩展程序」？
A：若你开启了 Chrome「安全浏览 · 增强型保护」，系统可能对尚未完成信任积累的开发者额外提示（官方说明：https://support.google.com/chrome_webstore/answer/2664769?hl=zh-Hans）。这不等于认定扩展有害，仍可点「继续安装」正常使用。
