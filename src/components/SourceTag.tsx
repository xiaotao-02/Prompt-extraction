...
const ON_DARK_CLASS: Record<PromptVersionSource, string> = {
  extracted: 'bg-emerald-500/20 text-emerald-300',
  edited: 'bg-amber-500/20 text-amber-300',
  restored: 'bg-violet-500/25 text-violet-300',
  refined: 'bg-fuchsia-500/20 text-fuchsia-300',
};
...
export function SourceTag({
  ...
  const cls = variant === 'onDark' ? ON_DARK_CLASS[source] : cfg.className;
  return <span className={`px-1.5 py-px rounded font-medium ${cls}`}>{cfg.label}</span>;
}
